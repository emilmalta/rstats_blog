...
an online map, which is useful for downloading geolocations of cities and 
...
```{r}
# Assign new prjection by EPSG:32642
greenland_good <- 
...
# Read localities
localities_raw <- 
  list.files("data/map_data/", full.names = TRUE, pattern = "*.csv") %>% 
  enframe() %>% 
...
You can sort of make out the coastline! This tells me that the data is loaded successfully, but I need to declare that it was loaded using EPSG:4326, and should be transformed to EPSG:32624. 
...
grl_map +
  geom_sf(data = localities_pop_sf,
          aes(size = population, color = type, fill = type))
```
...
The big red blob is the capitol city Nuuk, where I live. It is by far the most populous city.
...
Most of what I know about sf objects is straight from https://geocompr.robinlovelace.net/ 
